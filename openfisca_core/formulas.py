...
def set_input_divide_by_period(formula, period, array):
    ...
    if sub_periods_count > 0:
        ...
        raise ValueError("Inconsistent input : variable {0} has already been set for all months contained in period {1}, and value {2} provided for {1} does't match the total. This error may also be thrown if you try to call set_input twice for the same variable and period".format(holder.column.name, period, array))
